...
class TestSyncConfiguration:
    ...
    def test_settings_persistence(self):
        ...
        # Use temporary settings for this test
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
            temp_settings_path = temp_file.name
            
        try:
            # Create config with temporary settings file
            with patch.object(LocalSettingsManager, 'get_config_path', return_value=temp_settings_path):
                ...
                config2 = SyncConfiguration()
                backend_config = config2.get_coordination_backend_config()
